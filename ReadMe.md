...
须知，较为实用的情形，往往要求外界给出的【实用条目】之范系从上述【范_基础条目】扩展而得，特别是要按需对【`数据`】这一字段（或者说属性）做详实定义。以下是若干假想的例子：
...
1.  ##### 使用【`现成实用的双列`】部件时样式的写法
    ...
    1.  为采用本工具集自带的样式而做的配置或准备工作。具体而言，要在你的 Vuejs 项目的 `main.ts` 或 `main.js` 中采用本工具集自带的标准样式（或者说默认样式、基础样式）。
...
当采用【`单列`】部件以自行拼装功能模块时，难免要编写这样的工具函数（暂称【子】）。【子】接受两个【`单列`】的旧有【条目】列表，不妨暂称【甲】、【乙】两列，并将【甲】中各【条目】凡已【被选中故而凸显】者转移到【乙】中。最后，【子】返回新的两【条目】列表，供【甲】、【乙】两个【部件】采用。

又，【子】不应为【`单列`】的方法函数。换言之，【子】既不属于【甲】，也不属于【乙】。
...
本工具集中的【`现成实用的双列`】这一部件，在采用【`单列`】部件的同时，也用到了 `将某列选中的条目迁移至另一列` 这一工具函数。
...
>
    ...
    <template v-slot:界面皿-中央列>
        <template v-slot:界面皿-中央列之增补之功能区>……</tempalte>
    </template>
...
该【部件】有四个【界面皿】：`界面皿-总标题栏` 、 `界面皿-中央列` 、 `界面皿-中央列之增补之功能区` 和 `界面皿-底部内容栏` 。

其中，`界面皿-中央列之增补之功能区` 位于 `界面皿-中央列` 内部。即是说，如果自定义了 `界面皿-中央列` 之内容，则 `界面皿-中央列之增补之功能区` 不复存在。
