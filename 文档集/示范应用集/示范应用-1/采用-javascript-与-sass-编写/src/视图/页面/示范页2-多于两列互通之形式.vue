...
/**
 * @typedef {Wlc任意两列间转移条目_单列.泛范_实例可绑定之属性集<范_任意两列间转移条目_实际完整形式之条目> & { 其他数据: { 副标题之文本?: string; }; } 范_多列间转移条目功能_单列数据
 */
...
export default {
    ...
    methods: {
        ...
        构建各列之数据 ({ 总计列数 } = {}) {
            ...
            /** @type {boolean[]} */
            const 用于规定列数的临时数组 = new Array(总计列数).fill(false)
            ...
            const 各列数据之表 = 用于规定列数的临时数组.map(/** @returns {范_多列间转移条目功能_单列数据} */ (无用的值, 列编号) => {
                ...
                return {
                    ...
                    可成批自动绑定的属性: {
                        ...
                        无需底部的说明书功能区: false,
                        vue部件之定义_订制的主表条目: 故意要求该列之条目之形态为自定义形态 ? Wlc任意两列间转移条目_主列之条目之订制形态_Vue类之配置 : null,
                    },
    ...
    beforeMount () {
        ...
        this.$nextTick().then(() => {
            /** @type {范_多列间转移条目功能_多列数据之列表} */
            const 各列数据之表 = this.多列间转移条目功能之数据.各列数据之表
