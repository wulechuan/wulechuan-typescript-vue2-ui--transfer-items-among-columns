...
import Wlc任意两列互换数据_主表条目之订制形态 from '@/视图/部件/wlc-任意两列互换数据-主表条目-订制形态.vue'
...
type 范_多列互换数据功能之数据 = {
    ...
    各列数据之表: 范_多列互换数据功能_多列数据之列表;
};

type 范_多列互换数据功能_单列数据 = Wlc任意两列互换数据之单列.泛范_实例可绑定之属性集<范_双列互换数据_实际完整形式之条目> & {
    当下选中的所有条目之唯一标识之列表: 范_条目之唯一标识之列表;

    可成批自动绑定的属性: {
        所有条目之列表: 范_双列互换数据_实际完整形式之条目[];
    };
...
type 范_多列互换数据功能_多列数据之列表 = Array<范_多列互换数据功能_单列数据>;
...
const 一切可能的实用完整形式之条目之列表: 范_双列互换数据_实际完整形式之条目之列表 = 一切可能的条目去重后之列表.map(
...
export default class Page示范页2_多余2列互通之形式 extends Vue {
    ...
    private get 各列数据之表 (): 范_多列互换数据功能_多列数据之列表 {
    ...
    private 构建各列之数据 (列数: number): void {
        ...
        const 各列数据之表: 范_多列互换数据功能_多列数据之列表 = 用于规定列数的临时数组.map((无用的值, 列编号) => {
            ...
            return {
                ...
                可成批自动绑定的属性: {
                    ...
                    不应创建底部栏目: false,
                    vue部件之定义_订制的主表条目: 故意要求该列之条目之形态为自定义形态 ? Wlc任意两列互换数据_主表条目之订制形态 : null,
                },
    ...
    private 求_当下另有他列优先于本列采取视觉强调引导用户操作之 (本列之列表编号: number): boolean {
        const { 当下各列采用视觉强调与否之信号表 } = this.多列互换数据功能之数据.各列之公共数据
    ...
    private 每当某列之视觉强调之状态变动后(某列之列表编号: number, 该列当下正启用某种视觉强调: boolean): void {
        // console.debug(this.多列互换数据功能之数据.各列数据之表[某列之列表编号].可成批自动绑定的属性.本列之称谓, 该列当下正启用某种视觉强调)
        this.$set(this.多列互换数据功能之数据.各列之公共数据.当下各列采用视觉强调与否之信号表, 某列之列表编号, 该列当下正启用某种视觉强调)
    }
