<template>
    <div class="吴乐川-任意两列间转移条目-现成实用的双列" :class="{ '无需中央列之功能': !!无需中央列之功能 }">
        <header v-if="!无需总标题栏" class="总标题栏">
            <slot name="界面皿-总标题栏" v-bind="用于各界面皿之状态集"></slot>
        </header>

        <div class="容纳着双列的中央主体">
            <Wlc任意两列间转移条目_单列
                v-model="甲列之数据集.当下选中故而凸显的所有条目之唯一标识之列表"
                class="标准双列模式下的甲列"
                :应全面禁止交互="应全面禁止交互"
                ...
                :无需底部的说明书功能区="各列均无需底部的说明书功能区"
                :vue部件之定义_订制的主表条目="vue部件之定义_订制的主表条目"
                :应在控制台输出详尽的调试信息="应在控制台输出详尽的调试信息"
                @内部某元素之视觉强调之状态已变动="甲列之数据集.当下正在通过视觉强调动画引导用户 = $event"
            ></Wlc任意两列间转移条目_单列>

            <div v-if="无需中央列之功能" class="用以影响柔性布局之中央占位块"></div>
            <slot v-else name="界面皿-中央列" v-bind="用于各界面皿之状态集">
                <Wlc任意两列间转移条目_两列之间之竖栏_默认形态
                    :应全面禁止交互="应全面禁止交互"
                    :界面措辞_按钮甲="界面措辞_中央竖栏_内建按钮甲"
                    :界面措辞_按钮乙="界面措辞_中央竖栏_内建按钮乙"
                    :甲列当下选中的条目之总数="用于各界面皿之状态集.甲列当下选中的条目之总数"
                    :乙列当下选中的条目之总数="用于各界面皿之状态集.乙列当下选中的条目之总数"
                    @已点击按钮甲="将甲列选中的条目迁移至乙列()"
                    @已点击按钮乙="将乙列选中的条目迁移至甲列()"
                >
                    <template slot="界面皿-增补功能之功能区"><slot name="界面皿-中央列之增补功能之功能区"></slot></template>
                </Wlc任意两列间转移条目_两列之间之竖栏_默认形态>
            </slot>

            <Wlc任意两列间转移条目_单列
                v-model="乙列之数据集.当下选中故而凸显的所有条目之唯一标识之列表"
                class="标准双列模式下的乙列"
                :应全面禁止交互="应全面禁止交互"
        ...
        <footer v-if="!无需底部内容栏" class="底部内容栏">
                ...
                <div class="底部内容栏之默认内容">
                    <div class="功能块 功能块--针对甲列">
                        <div class="功能单体 功能单体-主体为按钮">
                            ...
                            >{{ 界面措辞_底部内容栏默认内容_按钮甲_最终采纳值 }}</button>
                    ...
                    <div class="功能块 功能块--针对乙列">
                        <div class="功能单体 功能单体-主体为按钮">
                            ...
                            >{{ 界面措辞_底部内容栏默认内容_按钮乙_最终采纳值 }}</button>
...
export default class Wlc任意两列间转移条目_现成实用的双列 extends Vue {
    ...
    @Prop() public readonly 无需总标题栏?: boolean
    @Prop() public readonly 无需中央列之功能?: boolean
    @Prop() public readonly 无需底部内容栏?: boolean
    ...
    @Prop() public readonly 界面措辞_中央竖栏_内建按钮甲?: string
    @Prop() public readonly 界面措辞_中央竖栏_内建按钮乙?: string
    @Prop() public readonly 界面措辞_底部内容栏默认内容_按钮甲?: string
    @Prop() public readonly 界面措辞_底部内容栏默认内容_按钮乙?: string
    @Prop() public readonly 各列均无需底部的说明书功能区?: boolean

    @Prop() public readonly 双列全部条目之总列表?: 范_基础条目之列表
    ...
    @Prop() public readonly 甲列初始选中故而凸显的条目之唯一标识之列表?: 范_条目之唯一标识之列表
    @Prop() public readonly 乙列初始选中故而凸显的条目之唯一标识之列表?: 范_条目之唯一标识之列表
    ...
    private get 用于各界面皿之状态集 (): 范_状态汇总数据 {
        ...
        const 甲列当下选中的条目之总数 = 甲列之数据集.当下选中故而凸显的所有条目之唯一标识之列表.length
        const 乙列当下选中的条目之总数 = 乙列之数据集.当下选中故而凸显的所有条目之唯一标识之列表.length
        return {
            甲列当下选中的条目之总数,
            乙列当下选中的条目之总数,
    ...
    private get 界面措辞_底部内容栏默认内容_按钮甲_最终采纳值 (): string {
        return 求可靠的文本(this.界面措辞_底部内容栏默认内容_按钮甲, '▶', false)
    }

    private get 界面措辞_底部内容栏默认内容_按钮乙_最终采纳值 (): string {
        return 求可靠的文本(this.界面措辞_底部内容栏默认内容_按钮乙, '◀', false)
    }
    ...
    @Watch('双列全部条目之总列表', { immediate: true })
    private 每当双列全部条目之总列表变动后 (): void {
        this.将所有候选条目分配到左右两列('【双列全部条目之总列表】变动了')
    }
    ...
    private 接受外界给出的甲乙两列勾选条目之配置 (): void {
        const {
            日志前缀,
            应在控制台输出详尽的调试信息,
            甲列初始选中故而凸显的条目之唯一标识之列表,
            乙列初始选中故而凸显的条目之唯一标识之列表,
        } = this

        if (应在控制台输出详尽的调试信息) {
            console.debug(`${日志前缀} 甲列初始选中故而凸显的条目之唯一标识之列表`, 甲列初始选中故而凸显的条目之唯一标识之列表)
            console.debug(`${日志前缀} 乙列初始选中故而凸显的条目之唯一标识之列表`, 乙列初始选中故而凸显的条目之唯一标识之列表)
        }

        if (Array.isArray(甲列初始选中故而凸显的条目之唯一标识之列表)) {
            this.甲列之数据集.当下选中故而凸显的所有条目之唯一标识之列表 = 甲列初始选中故而凸显的条目之唯一标识之列表
        }

        if (Array.isArray(乙列初始选中故而凸显的条目之唯一标识之列表)) {
            this.乙列之数据集.当下选中故而凸显的所有条目之唯一标识之列表 = 乙列初始选中故而凸显的条目之唯一标识之列表
        }
    }
    ...
    private 将所有候选条目分配到左右两列 (本次原因: string = '不明原因'): Promise<void> {
        ...
        const _分配条目 = () => {
            const 本次为求稳妥应对候选条目去重: boolean = 本次原因 === '【双列全部条目之总列表】变动了'
            ...
            let 本次采用的双列全部条目之总列表: 范_内用格式之条目之列表

            if (本次应采用去重好的缓存版本) {
                本次采用的双列全部条目之总列表 = 所有条目之列表之去重后的缓存版本!
            } else {
                let 给出的所有候选条目之原始列表: 范_基础条目之列表

                if (Array.isArray(this.双列全部条目之总列表)) {
                    给出的所有候选条目之原始列表 = this.双列全部条目之总列表
                } else {
                    给出的所有候选条目之原始列表 = []
                }

                本次采用的双列全部条目之总列表 = 给出的所有候选条目之原始列表.filter(基础条目 => !!基础条目 && typeof 基础条目 === 'object').map(基础条目 => {
            ...
            const 双列全部条目之总列表之去重好待缓存的版本: 范_内用格式之条目之列表 = []
            const 最终应位于甲列之条目之列表: 范_内用格式之条目之列表 = []
            const 最终应位于乙列之条目之列表: 范_内用格式之条目之列表 = []

            if (本次采用的双列全部条目之总列表.length > 0) {
                ...
                本次采用的双列全部条目之总列表.forEach(内部实用条目 => {
                    if (!内部实用条目) { return }
                    ...
                    if (本次为求稳妥应对候选条目去重) {
                        ...
                        双列全部条目之总列表之去重好待缓存的版本.push(内部实用条目)
                    }

                    const 该条目应位于乙列中: boolean = 乙列所有条目之唯一标识之列表_采纳值.includes(该条目之唯一标识)

                    if (该条目应位于乙列中) {
                        最终应位于乙列之条目之列表.push(内部实用条目)
                    } else {
                        最终应位于甲列之条目之列表.push(内部实用条目)
                    }
                })
                ...
                if (本次为求稳妥应对候选条目去重) {
                    ...
                    this.所有条目之列表之去重后的缓存版本 = 双列全部条目之总列表之去重好待缓存的版本
                }
            }
    ...
    private 将某列选中的条目迁移至对方列 (起列之数据集?: 范_单列之内部数据集): void {
        ...
        const 迁移后乙列应具有的所有条目之列表 = 迄列系乙列 ? 迁移后迄列所有条目之列表 : 迁移后起列所有条目之列表
        const 乙列之条目之唯一标识之列表: 范_条目之唯一标识之列表 = 迁移后乙列应具有的所有条目之列表.map(内部实用条目 => 内部实用条目.唯一标识)
