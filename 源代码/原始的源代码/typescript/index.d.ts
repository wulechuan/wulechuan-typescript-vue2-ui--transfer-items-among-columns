...
declare namespace Wlc任意两列间转移条目 {
    ...
    type 范_Vue动态部件之Is属性可接受的数据_但不可为部件名称字符串 = Vue构造函数 | Vue构造函数之配置项集
    ...
    class Wlc任意两列间转移条目_单列 extends Vue {
        ...
        private 每当外界给出的当下选中故而凸显的所有条目之唯一标识之列表变动后 (新值: 范_条目之唯一标识之列表): void
    ...
    namespace Wlc任意两列间转移条目_现成实用的双列 {
        type 泛范_实例可绑定之属性集<范_实际条目 extends 范_基础条目> = {
            ...
            甲列初始选中故而凸显的条目之唯一标识之列表?: 范_条目之唯一标识之列表;
            乙列初始选中故而凸显的条目之唯一标识之列表?: 范_条目之唯一标识之列表;
