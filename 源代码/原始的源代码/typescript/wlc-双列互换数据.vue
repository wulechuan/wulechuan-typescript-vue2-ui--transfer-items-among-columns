...
export default class Wlc双列互换数据 extends Vue {
    ...
    private 将所有候选条目分配到左右两列 (本次原因: string = '不明原因'): Promise<void> {
        ...
        const _分配条目 = () => {
            ...
            const 原本的甲列之所有条目之列表: 范_条目之列表 = [] = this.甲列之数据集.所有条目之列表
            const 原本的乙列之所有条目之列表: 范_条目之列表 = [] = this.乙列之数据集.所有条目之列表

            const 原本的甲列之所有条目之唯一标识之列表: 范_条目之唯一标识之列表 = 原本的甲列之所有条目之列表.map(条目 => 条目.唯一标识)
            const 原本的乙列之所有条目之唯一标识之列表: 范_条目之唯一标识之列表 = 原本的乙列之所有条目之列表.map(条目 => 条目.唯一标识)
