<template>
    <div class="吴乐川-任意两列间转移条目-单列" :class="本列之特征样式类名之配置" @click="每当根元素点击后($event)">
        <header class="列首">
            <div class="列标题栏">
                <slot name="界面皿-标题栏">
                    <div class="列标题栏之默认内容">
            ...
            <div class="功能块-条目过滤器">
                <input
                    ...
                    class="输入项 输入项-单行文字输入 条目过滤器-文本输入框"
            ...
            <div class="功能块-全选或全部取消选中">
                <label
                    ...
                    :class="与选中所有条目_含隐藏之条目_之交互相关之汇总数据.交互项之特征样式类名_三态勾选框_其根元素"
                >
                    <span
                        class="输入项 输入项-勾选项"
                        :class="与选中所有条目_含隐藏之条目_之交互相关之汇总数据.交互项之特征样式类名_三态勾选框_其勾选项之根元素"
                    >
                        <span class="勾选项视觉假体"></span>
                        <input
                            type="checkbox"
                            :checked="与选中所有条目_含隐藏之条目_之交互相关之汇总数据.三态勾选项之状态值 === '已勾选'"
                            :disabled="与选中所有条目_含隐藏之条目_之交互相关之汇总数据.应禁止三态勾选框之交互"
                            @change="每当选择所有条目或清除所有条目之选中状态_含隐藏之条目_之交互项动作时($event)"
                        />
                    </span>

                    <span class="输入项配文 输入项配文-勾选项">{{ 与选中所有条目_含隐藏之条目_之交互相关之汇总数据.三态勾选框之配文之措辞 }}</span>
                </label>

                <label
                    class="包裹着输入项 包裹着输入项-勾选项"
                    :class="与选中所有条目_仅列示之条目_之交互相关之汇总数据.交互项之特征样式类名_三态勾选框_其根元素"
                >
                    <span
                        class="输入项 输入项-勾选项"
                        :class="与选中所有条目_仅列示之条目_之交互相关之汇总数据.交互项之特征样式类名_三态勾选框_其勾选项之根元素"
                    >
                        <span class="勾选项视觉假体" ></span>
                        <input
                            type="checkbox"
                            :checked="与选中所有条目_仅列示之条目_之交互相关之汇总数据.三态勾选框之状态值 === '已勾选'"
                            :disabled="与选中所有条目_仅列示之条目_之交互相关之汇总数据.应禁止三态勾选框之交互"
                            @change="每当选择所有条目或清除所有条目之选中状态_仅列示之条目_之交互项动作时($event)"
                        />
                    </span>

                    <span class="输入项配文 输入项配文-勾选项">{{ 与选中所有条目_仅列示之条目_之交互相关之汇总数据.三态勾选框之配文之措辞 }}</span>
                </label>
            </div>
        ...
        <div class="功能块-条目列表皿">
            ...
            >
                <div class="消息文本-可以滚动的内容皿">
                    <article>
                        <p>{{ 用以筛选条目之关键词 ? '符合筛选条件的' : '候选' }}条目太多，<span class="尽量不换行之短语">达<em class="值 值-期望列示之条目总数">{{
            ...
            <ol v-else class="条目列表">
                ...
                >
                    <component
                        ...
                        :条目="采取内用格式之条目"
                        :所属列已禁止交互="应全面禁止交互_最终采纳值"
                        class="吴乐川-任意两列间转移条目-条目根"
...
type 范_交互项之三态勾选框_特征样式类名 = Wlc任意两列间转移条目.范_交互项之三态勾选框_特征样式类名;
type 范_三态勾选框之状态值 = Wlc任意两列间转移条目.范_三态勾选框之状态值;

// type 范_基础条目 = Wlc任意两列间转移条目.范_基础条目;
...
const 构建实用的条目总表_发起之原因_当下选中故而凸显的所有条目之唯一标识之列表已变动 = '当下选中故而凸显的所有条目之唯一标识之列表变动'
...
export default class Wlc任意两列间转移条目_单列 extends Vue {
    @Model('选中的条目已变动') public readonly 当下选中故而凸显的所有条目之唯一标识之列表?: 范_条目之唯一标识之列表
    ...
    private get 日志前缀 (): string {
        return `Vue 部件 <Wlc任意两列间转移条目_单列> “ ${this.本列之称谓_最终采纳值} ” ：`
    }
    ...
    private get 新增条目之插入规则_最终采纳值 (): 范_各列新增条目之插入规则 {
        ...
        if (typeof 外界给出值 === 'string' && 各列条目排序之规则所有允许的值之列表.includes(外界给出值)) {
            ...
            决定采纳的值 = 排序函数确已给出 ? '总是参与排序' : '总是在队尾追加'
        }
    ...
    private get 条目过滤器文本输入框元素之样式类名配置 (): 范_界面元素之样式类名之配置 {
        ...
        const 应给予视觉强调以引导用户来此: boolean = !应禁止条目过滤器交互 && 当下期望列示的条目过多故暂不列示任何条目 && !用以筛选条目之关键词
    ...
    private get 与选中所有条目_含隐藏之条目_之交互相关之汇总数据 () {
        return this.求_与全部选中或取消选中某些条目有关之三态勾选框之汇总数据(true)
    }

    private get 与选中所有条目_仅列示之条目_之交互相关之汇总数据 () {
        return this.求_与全部选中或取消选中某些条目有关之三态勾选框之汇总数据(false)
    }
    ...
    @Watch('当下选中故而凸显的所有条目之唯一标识之列表', { immediate: true })
    private 每当外界给出的当下选中故而凸显的所有条目之唯一标识之列表变动后 (): void {
        this.根据外界给出的条件构建实用的条目总表(构建实用的条目总表_发起之原因_当下选中故而凸显的所有条目之唯一标识之列表已变动)
    }
    ...
    private 每当外界给出的本列初始的用以过滤条目之配置变动后 (): void {
        ...
        if (本列初始的用以过滤条目之配置 instanceof RegExp) {
            this.用以筛选条目之关键词 = 本列初始的用以过滤条目之配置.toString()
        } else if (typeof 本列初始的用以过滤条目之配置 === 'string') {
            this.用以筛选条目之关键词 = 本列初始的用以过滤条目之配置
        }
    }

    @Watch('用以筛选条目之关键词', { immediate: true })
    private 每当用以筛选条目之关键词变动后 (): void {
    ...
    private 根据外界给出的条件构建实用的条目总表 (本次原因: 范_构建实用的条目总表_发起之原因): Promise<boolean> {
        const { 日志前缀, 应在控制台输出详尽的调试信息 } = this

        const _构建实用的条目总表 = () => {
            let 给出的所有条目之原始无序列表: 范_内用格式之条目之列表
            let 当下选中故而凸显的所有条目之唯一标识之列表: 范_条目之唯一标识之列表
            ...
            if (Array.isArray(this.当下选中故而凸显的所有条目之唯一标识之列表)) {
                当下选中故而凸显的所有条目之唯一标识之列表 = this.当下选中故而凸显的所有条目之唯一标识之列表
            } else {
            ...
            Object.keys(当下选中故而凸显的所有条目之唯一标识之列表).map(某条目之唯一标识 => {
            ...
            if (应在控制台输出详尽的调试信息) {
                ...
                console.debug(
                    ...
                    '\n\t\t选中的所有条目之列表：', 当下选中故而凸显的所有条目之唯一标识之列表.length, '条。',
                    当下选中故而凸显的所有条目之唯一标识之列表.length === 0 ? '' : `\n\t\t\t有：${当下选中故而凸显的所有条目之唯一标识之列表.slice(0, 在日志中列举条目之数量之上限).join('、')}${当下选中故而凸显的所有条目之唯一标识之列表.length > 在日志中列举条目之数量之上限 ? '……' : '。'}`
                )
            }
            ...
            const 应保留的旧有条目之列表: 范_内用格式之条目之列表 = 旧有的所有条目之列表.filter(条目 => {
                ...
                if (该条目应保留) {
                    const 该条目此后应选中 = !该条目已禁止选择 && 当下选中故而凸显的所有条目之唯一标识之列表.includes(该条目之唯一标识)
    ...
    private 求_与全部选中或取消选中某些条目有关之三态勾选框之汇总数据 (本次统计时应将暂不可见的条目一并计入?: boolean) {
        type 范_三态勾选框之配文之措辞 = (
            | '--'
            | '无可勾选项'
            | '选中所有条目（含未列示条目）'
        ...
        let 三态勾选框之配文之措辞: 范_三态勾选框之配文之措辞

        const 交互项之特征样式类名_三态勾选框_其根元素: 范_交互项之三态勾选框_特征样式类名 = {}
        const 交互项之特征样式类名_三态勾选框_其勾选项之根元素: 范_交互项之三态勾选框_特征样式类名 = {}
        ...
        const 当下列示着的所有条目数: number = 当下列示着的所有条目之列表.length
        const 当所有可交互之条目数: number = 所有未禁止交互之条目之列表.length
        const 当下列示着的所有可交互之条目数: number = 当下列示着的所有未禁止交互之条目之列表.length

        const 所有条目均不可交互: boolean = 本次统计时应将暂不可见的条目一并计入
            ? 当所有可交互之条目数 === 0
            : 当下列示着的所有可交互之条目数 === 0

        const 所有条目均已列示: boolean = 当下列示着的所有条目数 === 所有条目之总数
        ...
        const 无可交互之条目: boolean = 所有条目均不可交互 || 本次统计之条目之含义并非所有条目但数目恰与总条目数相同_哪怕总条目数为零
        const 外界迫使该勾选框禁止交互: boolean = 应全面禁止交互_最终采纳值
        const 应禁止三态勾选框之交互: boolean = 无可交互之条目 || 外界迫使该勾选框禁止交互

        if (无可交互之条目) {
            if (本次统计时应将暂不可见的条目一并计入) {
                三态勾选框之配文之措辞 = '无可勾选项'
            } else {
            ...
            if (三态勾选框之状态值 === '未勾选') {
                三态勾选框之配文之措辞 = `选中所有条目${界面措辞之统一后缀}`
            } else if (三态勾选框之状态值 === '已勾选') {
                三态勾选框之配文之措辞 = `清除所有选择${界面措辞之统一后缀}`
            } else {
                三态勾选框之配文之措辞 = `选中所有条目${界面措辞之统一后缀}`
            }
        }
        ...
        if (应禁止三态勾选框之交互) {
            if (无可交互之条目) {
                交互项之特征样式类名_三态勾选框_其根元素['不可用'] = true
                交互项之特征样式类名_三态勾选框_其勾选项之根元素['不可用'] = true
            } else {
                交互项之特征样式类名_三态勾选框_其根元素['已禁止交互'] = true
                交互项之特征样式类名_三态勾选框_其勾选项之根元素['已禁止交互'] = true
            }
        }

        if (三态勾选框之状态值 === '未勾选') {
            交互项之特征样式类名_三态勾选框_其根元素['未勾选'] = true
            交互项之特征样式类名_三态勾选框_其勾选项之根元素['未勾选'] = true
        } else if (三态勾选框之状态值 === '已勾选') {
            交互项之特征样式类名_三态勾选框_其根元素['已勾选'] = true
            交互项之特征样式类名_三态勾选框_其勾选项之根元素['已勾选'] = true
        } else {
            交互项之特征样式类名_三态勾选框_其根元素['已部分勾选'] = true
            交互项之特征样式类名_三态勾选框_其勾选项之根元素['已部分勾选'] = true
        }
        ...
        const 汇总数据 = {
            ...
            应禁止三态勾选框之交互,
            三态勾选框之状态值,
            交互项之特征样式类名_三态勾选框_其根元素,
            交互项之特征样式类名_三态勾选框_其勾选项之根元素,
        }
...
// 目前推荐的做法是，始终提醒本工具之使用者在其 Vue 项目的的 `main.ts` 或 `main.js` 中加载 `0-基础.styl` 文件。
// 由此，下方两行代码被注释掉，即作废。
