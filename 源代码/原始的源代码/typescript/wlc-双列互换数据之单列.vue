    ...
    <div class="column-of-list" :class="本列之特征样式类名之配置">
        ...
        <div class="column-list-container">
            ...
            >
                ...
                }}</em>条，</span>已逾预设的<span class="尽量不换行之短语"><em class="条目总数之限制数">{{
                    允许列示的条目数之上限_最终采纳值
                }}</em>条之限。</span>为确保程序不致僵死，暂不列示任何条目。</p>
...
export default class Wlc双列互换数据之单列 extends Vue {
    ...
    private get 当下列示着的所有未禁止交互之条目之列表 (): 范_条目之列表 {
        return this.当下列示着的所有条目之列表.filter(条目 => !条目.已禁止交互)
    }
    ...
    private 求与选中所有条目之交互相关的汇总数据 (应将暂不可见的条目一并计入?: boolean) {
        const 所有参与统计之条目: 范_条目之列表 = 应将暂不可见的条目一并计入
            ? this.所有未禁止交互之条目之列表
            : this.当下列示着的所有未禁止交互之条目之列表

        const 三态勾选框之状态值: 范_三态勾选框之状态值 = 应将暂不可见的条目一并计入
        ...
        // console.debug(`${this.日志前缀}\n\t含隐藏条目：`, 应将暂不可见的条目一并计入, '\t\t状态：', 汇总数据.三态勾选框之状态值)
