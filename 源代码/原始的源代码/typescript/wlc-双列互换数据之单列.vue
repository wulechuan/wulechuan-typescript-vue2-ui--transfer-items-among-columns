    ...
    private 根据外界给出的条件构建实用的条目总表 (本次原因: string = '不明原因'): Promise<void> {
        ...
        const _构建实用的条目总表 = () => {
            ...
            const 应保留的旧有条目之列表: 范_条目之列表 = 旧有的所有条目之列表.filter(条目 => {
                ...
                if (该条目应保留) {
                    const 该条目此后应选中 = !该条目已禁止交互 && 选中的所有条目之唯一标识之列表.includes(该条目之唯一标识)
            ...
            const 应追加至新列表的条目之列表: 范_条目之列表 = 给出的所有条目之原始无序列表.filter(条目 => !!条目 && !旧有的所有条目之唯一标识之列表.includes(条目.唯一标识))

            应追加至新列表的条目之列表.forEach(条目 => {
                条目.已选中 = false
                if (!条目.已禁止交互) { 条目.已禁止交互 = false } // 防止外界省略该值，在此初始化之。
            })
