    ...
    <div class="吴乐川-双列互换数据-单列" :class="本列之特征样式类名之配置">
        <header class="列首">
            ...
            <div class="功能块-全选或全部取消选中">
                ...
                ></span><span class="输入项配文 输入项配文-勾选项">{{ 与选中所有条目_仅列示之条目_之交互相关的汇总数据.交互项之界面措辞 }}</span></label>
            </div>
        ...
        <div class="功能块-条目列表皿">
            <div
                ...
                class="条目列表之提示语"
            >
                <p>{{ 用以过滤条目之关键词 ? '符合筛选条件的' : '候选' }}条目太多，<span class="尽量不换行之短语">达<em class="期望列示之条目总数">{{
...
export default class Wlc双列互换数据之单列 extends Vue {
    ...
    private 根据外界给出的条件构建实用的条目总表 (本次原因: string = '不明原因'): Promise<boolean> {
        const { 日志前缀 } = this
        ...
        if (!this.所有条目之列表_最终采纳值_求解之期待) {
            ...
            this.所有条目之列表_最终采纳值_求解之期待 = new Promise<boolean>((令期待落实, 令期待落空) => {
