...
let wlc双列互换数据 = class wlc双列互换数据 extends Vue {
    ...
    get leftMatchedItems() {
        const { allItems, filteringKeyword, } = this.左列之状态;
    ...
    get rightMatchedItems() {
        const { allItems, filteringKeyword, } = this.右列之状态;
        let 所有匹配的条目 = allItems;
        if (filteringKeyword) {
            const 用以匹配条目的正则表达式 = new RegExp(`${filteringKeyword}`, 'i');
            所有匹配的条目 = allItems.filter(条目 => 用以匹配条目的正则表达式.test(条目.在界面展示时的称谓));
    ...
    generateItemsOfBothColumns() {
        ...
        candidates.forEach(原始条目 => {
            ...
            const 条目 = Object.assign(Object.assign({}, 原始条目), { 已选中: false });
            const 该条目应位于右列中 = rightsideValues.includes(该条目之唯一标识);
            // if (该条目应位于右列中) {
